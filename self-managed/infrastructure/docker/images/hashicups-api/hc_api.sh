...
EOF
    ...
    if [ "${REGEN_CONFIG}" == true ]; then
        ...
        if [ "${CONSUL_NODE}" == true ] || [ "${CONSUL_SERVICE}" == true ]; then
            ...
            tee /home/admin/conf.json << EOF
{
	"db_connection": "host=${_DATABASE_ADDRESS}.${DNS_LOOKUP}.${_DC}.${_DOMAIN} port=5432 user=postgres password=p05tgr35 dbname=products sslmode=disable",
	"bind_address": ":9090",
...
EOF
            ...
            tee /home/admin/conf.json << EOF
{
	"db_connection": "host=${_DATABASE_ADDRESS} port=5432 user=postgres password=p05tgr35 dbname=products sslmode=disable",
	"bind_address": ":9090",
	"metrics_address": ":9103"
}
EOF
