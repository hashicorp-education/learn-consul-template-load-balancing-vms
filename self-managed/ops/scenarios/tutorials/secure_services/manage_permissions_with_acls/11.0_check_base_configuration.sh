...
_check_value_in_range "gateway-api" "${api_gw_NUMBER}" "${_api_gw_MIN}" "${_api_gw_MAX}"
...
log "Testing HashiCups instances."
...
if [ "${ENABLE_SERVICE_MESH}" != "true" ]; then
    log_err "Service mesh not enabled. Scenario cannot be demonstrated."
    exit 1
fi
